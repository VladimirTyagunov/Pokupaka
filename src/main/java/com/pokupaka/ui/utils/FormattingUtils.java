...
public class FormattingUtils {
	 ...
	 */
	public static final TemporalField WEEK_OF_YEAR_FIELD = WeekFields.of(PokupakaAppConst.APP_LOCALE).weekOfWeekBasedYear();
	...
	public static String getFullMonthName(LocalDate date) {
		return date.getMonth().getDisplayName(TextStyle.FULL, PokupakaAppConst.APP_LOCALE);
	}

	public static String formatAsCurrency(int valueInCents) {
		return NumberFormat.getCurrencyInstance(PokupakaAppConst.APP_LOCALE).format(BigDecimal.valueOf(valueInCents, 2));
	}

	public static DecimalFormat getUiPriceFormatter() {
		DecimalFormat formatter = new DecimalFormat("#" + DECIMAL_ZERO,
				DecimalFormatSymbols.getInstance(PokupakaAppConst.APP_LOCALE));
